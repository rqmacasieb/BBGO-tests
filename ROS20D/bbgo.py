...
def inner_opt(iitidx):
    ...
    pyemu.os_utils.start_workers(tmpl_in, "pestpp-mou", 
                                 "ros_bbgo.pst", num_workers=num_workers, 
...
def create_gpmodel(X, Y):
    ...
    gpr.buildGP(X, Y, fname=os.path.join(tmpl_in, f"gp_0.gp"), kernel='matern32')
...
def resample(inner_dirs, outer_dirs):
    ...
    kmeans = KMeans(n_clusters=pop_size-1).fit(augmented_training_dv.drop(columns='real_name'))
