...
if __name__ == "__main__":
    ...
    # Save the best x values
    min_obj_value = float('inf')
    min_x_values = None
    all_min_x = []

    for i, (obj_val, x_val) in enumerate(zip(all_outputs, all_inputs)):
        if obj_val < min_obj_value:
            min_obj_value = obj_val
            min_x_values = x_val
        all_min_x.append(min_x_values)
    ...
    # Save the x values corresponding to minimum objectives
    inputs_repo_df = pd.DataFrame(all_min_x, columns=[f'x_{i}' for i in range(len(all_min_x[0]))])
    inputs_repo_df.to_csv('inputs_archive_summary.csv', index_label='iteration')
