...
pardata = pd.read_csv("./template/pest/prams_bbgo.par_data.csv", index_col=0)

adj_dvs = pardata.loc[pardata["partrans"] != "fixed"]
bounds = adj_dvs.loc[:, ["parlbnd", "parubnd"]].values
dv_names = adj_dvs.index.tolist()
...
def generate_lhstrainingset(seed):
    np.random.seed(seed)
    
    n_samples = 500
    n_dimensions = len(adj_dvs)
    ...
    complete_samples = pd.DataFrame(index=member_names, columns=pardata.index)
    
    complete_samples[dv_names] = samples_df
    
    fixed_params = pardata.loc[pardata["partrans"] == "fixed"]
    for param in fixed_params.index:
        complete_samples[param] = fixed_params.loc[param, "parval1"]
    
    complete_samples.rename_axis("real_name", inplace=True)
    complete_samples.to_csv("./template/pest/gp.lhs.dv_pop.csv", index=True)



def generate_lhsstarter(seed):
    ...
    # Create a complete DataFrame with all parameters
    complete_samples = pd.DataFrame(index=member_names, columns=pardata.index)
    
    # Fill in the sampled values for adjustable parameters
    complete_samples[dv_names] = samples_df
    
    # Fill in the fixed values from pardata
    fixed_params = pardata.loc[pardata["partrans"] == "fixed"]
    for param in fixed_params.index:
        complete_samples[param] = fixed_params.loc[param, "parval1"]
    
    complete_samples.rename_axis("real_name", inplace=True)
    complete_samples.to_csv("./template/pest/starter.dv_pop.csv", index=True)
