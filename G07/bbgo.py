...
def inner_opt(iitidx):
    ...
    pyemu.os_utils.start_workers(tmpl_in, "pestpp-mou", 
                                 "g07_bbgo.pst", num_workers=num_workers, 
...
def create_gpmodel(X, Y):
    ...
    obj = Y.drop(columns=['real_name'])['func'].values
    g1 = Y.drop(columns=['real_name'])['g1'].values
    g2 = Y.drop(columns=['real_name'])['g2'].values
    g3 = Y.drop(columns=['real_name'])['g3'].values
    g4 = Y.drop(columns=['real_name'])['g4'].values
    g5 = Y.drop(columns=['real_name'])['g5'].values
    g6 = Y.drop(columns=['real_name'])['g6'].values
    g7 = Y.drop(columns=['real_name'])['g7'].values
    g8 = Y.drop(columns=['real_name'])['g8'].values

    gpr.buildGP(X, obj, fname=os.path.join(tmpl_in, f"gp_obj.gp"), kernel='matern32')
    gpr.buildGP(X, g1, fname=os.path.join(tmpl_in, f"gp_g1.gp"), kernel='matern32')
    gpr.buildGP(X, g2, fname=os.path.join(tmpl_in, f"gp_g2.gp"), kernel='matern32')
    gpr.buildGP(X, g3, fname=os.path.join(tmpl_in, f"gp_g3.gp"), kernel='matern32')
    gpr.buildGP(X, g4, fname=os.path.join(tmpl_in, f"gp_g4.gp"), kernel='matern32')
    gpr.buildGP(X, g5, fname=os.path.join(tmpl_in, f"gp_g5.gp"), kernel='matern32')
    gpr.buildGP(X, g6, fname=os.path.join(tmpl_in, f"gp_g6.gp"), kernel='matern32')
    gpr.buildGP(X, g7, fname=os.path.join(tmpl_in, f"gp_g7.gp"), kernel='matern32')
    gpr.buildGP(X, g8, fname=os.path.join(tmpl_in, f"gp_g8.gp"), kernel='matern32')
    print(f"\n{datetime.datetime.now()}: GP models saved. \n")
...
def inner_prep(inner_dirs, outer_dirs):
    ...
    curr_opt_obs.to_csv(os.path.join(tmpl_in, "curr_opt.csv"), index=False)

    #copy previous outer repo update dv and obs files
    if outer_dirs[-1].endswith("_0"):      
        restart_dv = pd.read_csv(os.path.join(tmpl_in, "starter.dv_pop.csv"))
    else:
        restart_dv = pd.read_csv(os.path.join(tmpl_in, "initial.dv_pop.csv"))
    ...
    create_gpmodel(training_dv, training_obs)
...
def prep_templates():
    ...
    constraints = [f'g{i}' for i in range(1, 9)]
    obs.loc[constraints, 'obgnme'] = 'l_than'  
    ...
    if CASE == "MinVar":
        ...
        raise ValueError("CASE variable must be set to either 'MinVar' or 'MaxVar'")
    pf_constraints = [f'pf_g{i}' for i in range(1, 9)]
    obs.loc[pf_constraints, 'obgnme'] = 'g_than'
    obs.loc[pf_constraints, 'obsval'] = Pf_m
    # obs.loc['ei', 'obgnme'] = 'g_than'
...
def get_outer_outputs(outer_dir,output_dir):
    ...
    csv_files = [
        file for file in glob.glob(os.path.join(outer_dir, '*.csv'))
...
def get_inner_outputs(inner_dir, output_dir):
    ...
    csv_files = [
        file for file in glob.glob(os.path.join(inner_dir, '*.csv'))
...
if __name__ == "__main__":
    ...
    for i in range(nmax_outer+1):
        ...
        get_outer_outputs(outer_dirs[-1], output_dir)
        ...
        get_inner_outputs(inner_dirs[-1], output_dir)
