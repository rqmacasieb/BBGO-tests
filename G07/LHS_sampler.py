...
def generate_lhstrainingset(seed):
    ...
    bounds = np.array([[-10, 10] for i in range(n_dimensions)])
    ...
    samples_df.to_csv("./template/gp.lhs.dv_pop.csv", index=True)
...
def generate_lhsstarter(seed):
    ...
    samples_df = pd.DataFrame(samples_maximin, index=member_names, columns=dv_names)
    samples_df.rename_axis("real_name", inplace=True)
    samples_df.to_csv("./template/starter.dv_pop.csv", index=True)
